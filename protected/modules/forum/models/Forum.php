...
class Forum extends CActiveRecord {
    ...
    public function getBreadcrumbs($currentlink = false) {
        ...
        $breadcrumbs = array_merge(
                array('Forum' => array('/forum')), array_reverse($breadcrumbs)
        );

        if (!$this->isNewRecord) {
            $breadcrumbs = array_merge($breadcrumbs, $currentlink ? array(CHtml::encode($this->title) => array('/forum/forum/view', 'id' => $this->id)) : array(CHtml::encode($this->title))
            );
        }
        return $breadcrumbs;
    }
    ...
    public function renderForumCell() {
        $result = '<div class="name">' . CHtml::link(CHtml::encode($this->title), $this->url) . '</div>' .
    ...
    public function renderLastpostCell() {
        ...
        $authorlink = CHtml::link(CHtml::encode($author->username), Yii::app()->createUrl('/sUser/viewAuthenticated', array('id' => $author->id)));
