...
class EmailMustache {
    ...
    protected $_throwsExceptions = array(
        EmailMustacheException::UNKNOWN_VARIABLE => false,
        EmailMustacheException::UNCLOSED_SECTION => true,
        EmailMustacheException::UNEXPECTED_CLOSE_SECTION => true,
        EmailMustacheException::UNKNOWN_PARTIAL => false,
        EmailMustacheException::UNKNOWN_PRAGMA => true,
    );
        ...
        if (isset($options['pragmas'])) {
            foreach ($options['pragmas'] as $pragma_name => $pragma_value) {
                if (!in_array($pragma_name, $this->_pragmasImplemented, true)) {
                    throw new EmailMustacheException('Unknown pragma: ' . $pragma_name, EmailMustacheException::UNKNOWN_PRAGMA);
                }
            }
    ...
    protected function _findSection($template) {
            ...
            switch ($type) {
                ...
                case '/':
                    if (empty($section_stack) || ($tag_name !== array_pop($section_stack))) {
                        if ($this->_throwsException(EmailMustacheException::UNEXPECTED_CLOSE_SECTION)) {
                            throw new EmailMustacheException('Unexpected close section: ' . $tag_name, EmailMustacheException::UNEXPECTED_CLOSE_SECTION);
                        }
                    }
        ...
        if (!empty($section_stack)) {
            if ($this->_throwsException(EmailMustacheException::UNCLOSED_SECTION)) {
                throw new EmailMustacheException('Unclosed section: ' . $section_stack[0], EmailMustacheException::UNCLOSED_SECTION);
            }
        }
    }
     ...
     * @param mixed $matches
     * @return void
     * @throws EmailMustacheException unknown pragma
     */
     ...
     *
     * Expects exceptions to be EmailMustacheException error codes (i.e. class constants).
     *
     * @access protected
     ...
     * @throws EmailMustacheException Unmatched section tag encountered.
     * @return string
     */
     ...
     * @access protected
     * @param string $tag_name
     * @throws EmailMustacheException Unknown variable name.
     * @return string
     */
    ...
    protected function _findVariableInContext($tag_name, $context) {
        ...
        if ($this->_throwsException(EmailMustacheException::UNKNOWN_VARIABLE)) {
            throw new EmailMustacheException("Unknown variable: " . $tag_name, EmailMustacheException::UNKNOWN_VARIABLE);
        } else {
            return '';
        }
    }
     ...
     * @access protected
     * @param string $tag_name
     * @throws EmailMustacheException Unknown partial name.
     * @return string
     */
    protected function _getPartial($tag_name) {
        ...
        if ($this->_throwsException(EmailMustacheException::UNKNOWN_PARTIAL)) {
            throw new EmailMustacheException('Unknown partial: ' . $tag_name, EmailMustacheException::UNKNOWN_PARTIAL);
        } else {
            return '';
        }
    }
...
/**
 * EmailMustacheException class.
 *
 * @extends Exception
 */
class EmailMustacheException extends Exception {
