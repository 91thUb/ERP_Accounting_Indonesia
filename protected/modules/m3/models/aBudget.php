...
class aBudget extends BaseModel {
    ...
    public static function mainComponent($id = null) { //$id = for filterisasi by Spesific COmponent
        ...
        if (Yii::app()->user->name != 'admin')
            $criteria->addInCondition('department_id', array(sUser::model()->myGroupRoot));
    ...
    public function getTopTenBudgetCP() {
        ...
        foreach ($models as $model) {
            $returnarray[] = array('id' => $model->id, 'label' => $model->name . ' (' . $model->end_balance->balancef() . ')', 'icon' => 'list-alt', 'url' => array('/m3/aBudget', 'id' => $model->id));
        }
