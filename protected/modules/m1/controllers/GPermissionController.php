...
class GPermissionController extends Controller {
            ...
            if ($model->validate()) {
                ...
                if ($model->report_id == 1) {  //Detail
                    ...
                    $sql = "SELECT a.employee_name, a.company, a.department, a.level, a.join_date, a.job_title,
						...
						INNER JOIN g_bi_person a ON p.parent_id = a.id
						WHERE a.company_id = " . sUser::model()->myGroup . " AND a.employee_status NOT IN ('Resign','End of Contract','Black List','Termination')
    ...
    public function actionPermissionCalendar() {
        $this->render('permissionCalendar', array());
    }
    ...
    public function actionPermissionCalendarAjax() {
        ...
        foreach ($rows as $row) {
            ...
            $detail['allDay'] = true;
            $detail['url'] = Yii::app()->createUrl('/m1/gPermission/view', array("id" => $row['id']));
            $items[] = $detail;
        }
