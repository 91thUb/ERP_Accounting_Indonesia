...
class permissionSummaryByDept extends fpdf {
    ...
    function myheader($rows, $w, $model) {
        ...
        $this->Cell($w[1], 4, 'Tanggal', 1, 0, 'C');
        $this->Cell($w[2], 4, 'Nama', 1, 0, 'C');
        $this->Cell($w[3], 4, 'Departmen', 1, 0, 'C');
        $this->Cell($w[4], 4, 'Alasan', 1, 0, 'C');
        $this->Ln();
    }

    function report($rows, $model) {
        $w = array(8, 40, 49, 50, 130);
        ...
        foreach ($rows as $row) {
            ...
            $this->Cell($w[1], 7, date('d-m-Y h:i', strtotime($row['start_date'])), 'L');
            $this->Cell($w[2], 7, $row['employee_name'], 'L');
            $this->Cell($w[3], 7, substr($row['department'], 0, 30), 'LR');
            $this->Cell($w[4], 7, $row['permission_reason'], 'LR');
            $this->Ln();
            $counter++;
