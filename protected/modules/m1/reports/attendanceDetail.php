...
class attendanceDetail extends fpdf {
    ...
    function Footer() {
        ...
        $this->Cell(0, 10, 'Printed Date: ' . Yii::app()->dateFormatter->format("dd-MM-yyyy", time()) . '                        ' .
                'Page: ' . $this->PageNo() . '/{nb}' . '                        ' .
                'Issued By: APHRIS - Agung Podomoro', 0, 0, 'C');
    }
    ...
    function report($models) {
        ...
        $w = array(20, 34, 9, 9, 9, 9, 9, 7, 7, 7, 10, 10, 8, 8, 34);
        ...
        $_overtimein = $_overtimeout = $_countlatein = $_countlateout = "00:00";

        $totalworkhour = "00:00:00";

        foreach ($models as $model) {
            $currentworkhour = peterFunc::get_time_difference($model->actualIn, $model->actualOut);
