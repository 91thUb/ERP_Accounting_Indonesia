...
if (!Yii::app()->user->isGuest) {
    ...
    if (!Yii::app()->cache->get('hierarchy1m4' . Yii::app()->user->id)) {
        if (Yii::app()->user->name == 'admin') {
            ...
            $criteria1->compare('name', 'm4', true, 'OR');
        ...
        Yii::app()->cache->set('hierarchy1m4' . Yii::app()->user->id, $Hierarchy, 86400, $dependency);
    } else
        $Hierarchy = Yii::app()->cache->get('hierarchy1m4' . Yii::app()->user->id);

    if (!Yii::app()->cache->get('hierarchy2m4' . Yii::app()->user->id)) {
        foreach ($Hierarchy as $Hierarchy) {
            $models = menu::model()->findByPk($Hierarchy->id);
            $items[] = $models->getListed();
        }
        Yii::app()->cache->set('hierarchy2m4' . Yii::app()->user->id, $items, 86400, $dependency);
    } else
        $items = Yii::app()->cache->get('hierarchy2m4' . Yii::app()->user->id);
