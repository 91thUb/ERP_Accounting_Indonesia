...
class sNotification extends BaseModel {
    ...
    public static function getNotifCount() {
        ...
        if (Yii::app()->user->name == "admin" || sUser::model()->rightCountM > 2 || !Yii::app()->user->checkAccess('HR ESS Staff')) {
            ...
            if ((int) $total >= 200)
                $total = "200+";
            $unread = (self::model()->unreadCount != 0) ? CHtml::tag("span", array('style' => 'font-size:inherit', 'class' => 'badge badge-info'), $total) : "";
        }
    ...
    public static function getNotifMessageCount() {
        ...
        if (Yii::app()->user->name == "admin" || sUser::model()->rightCountM > 2 || !Yii::app()->user->checkAccess('HR ESS Staff')) {
            ...
            if ((int) $total >= 200)
                $total = "200+";
            $unread = (self::model()->unreadCount != 0 || Mailbox::newMsgs(Yii::app()->user->id) != 0) ? CHtml::tag("span", array('style' => 'font-size:inherit', 'class' => 'badge badge-info'), $total) : "";
        } else {
            $total = Mailbox::newMsgs(Yii::app()->user->id);
            if ((int) $total >= 200)
                $total = "200+";
            $unread = (Mailbox::newMsgs(Yii::app()->user->id) != 0) ? CHtml::tag("span", array('style' => 'font-size:inherit', 'class' => 'badge badge-info'), $total) : "";
        }
